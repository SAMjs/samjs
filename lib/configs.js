(function() {
  ...
  module.exports = function(samjs) {
    var Config, fs, hooknames, initiateHooks, syncHooks;
    fs = samjs.Promise.promisifyAll(require("fs"));
    hooknames = ["afterCreate", "afterGet", "afterSet", "afterTest", "beforeCreate", "beforeGet", "beforeSet", "beforeTest", "before_Set"];
    syncHooks = ["afterCreate", "beforeCreate"];
    initiateHooks = function(config) {
      config._hooks = {};
      return hooknames.forEach(function(hookname) {
        if (syncHooks.indexOf(hookname) > -1) {
          config._hooks[hookname] = function(arg) {
            var hook, i, len, ref;
            ref = config._hooks[hookname]._hooks;
          ...
          config._hooks[hookname] = function() {
            var args, hook, i, len, promise, ref;
        ...
        return config._hooks[hookname]._hooks = [];
      });
    };
    Config = (function() {
      ...
      Config.prototype.addHook = function(name, hook) {
        var add, i, isPush, len, results, singleHook;
        if (hooknames.indexOf(name) > -1) {
          ...
          add = (function(_this) {
            return function(hook) {
              if (samjs.util.isFunction(hook)) {
                if (isPush) {
                  return _this._hooks[name]._hooks.push(hook);
                } else {
                  return _this._hooks[name]._hooks.unshift(hook);
                }
              }
            };
          })(this);
          if (samjs.util.isArray(hook)) {
            results = [];
            for (i = 0, len = hook.length; i < len; i++) {
              singleHook = hook[i];
              results.push(add(singleHook));
            }
            return results;
          } else {
            return add(hook);
      ...
      Config.prototype._set = function(newData) {
        return this._test(newData).then((function(_this) {
          return function() {
