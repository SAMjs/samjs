(function() {
  ...
  module.exports = function(samjs) {
    return samjs.startup = function(server) {
      ...
      samjs.exposeInterface = function(name, listener) {
      ...
      samjs.started = samjs.state.ifConfigured().then(function() {
      ...
      }), install.install).then(install.finish).then(function() {
        ...
        for (name in ref) {
          ...
          results.push((function() {
            ...
            for (name in ref1) {
              listener = ref1[name];
              results1.push(model.removeInterface[name] = samjs.exposeInterface(name, listener.bind(model)));
            }
            return results1;
          })());
        }
