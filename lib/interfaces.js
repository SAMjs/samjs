  ...
  module.exports = function(samjs) {
    ...
    return samjs.interfaces = new (Interfaces = (function() {
      ...
      Interfaces.prototype.expose = function(name) {
        ...
        exposeInterface = function(itf, name, binding) {
          var tempitf;
          tempitf = binding ? itf.bind(binding) : itf;
          ...
          return function(interfaces, name, binding) {
            ...
            if (samjs.util.isArray(interfaces)) {
              ...
              for (i = 0, len = interfaces.length; i < len; i++) {
                itf = interfaces[i];
                results.push(_this._closers[name].push(exposeInterface(itf, name, binding)));
              }
              return results;
            } else {
              return _this._closers[name].push(exposeInterface(interfaces, name, binding));
            }
          };
        })(this);
        if (name) {
